...
export class LIDMappingStore {
    ...
    async storeLIDPNMappings(pairs) {
        ...
        for (const { lid, pn } of pairs) {
            ...
            this.logger.info({ pnUser, lidUser }, 'New LID-PN mapping detected');
        ...
        if (Object.keys(pairMap).length > 0) {
            this.logger.info({ count: Object.keys(pairMap).length }, 'Successfully stored new LID-PN mappings');
        }
    }
    /**
    ...
    async getPNForLID(lid) {
        ...
        this.logger.debug({ lid, pnJid }, 'Found reverse LID → PN mapping');
        return pnJid;
    }
}

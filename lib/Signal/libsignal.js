...
function signalStorage({ creds, keys }, lidMapping, logger) {
    ...
    const resolveLIDSignalAddress = async (id) => {
        if (id.includes('.') && !id.includes('-')) {
            ...
            if (domainType === WAJIDDomains.LID || domainType === WAJIDDomains.HOSTED_LID || !/^\d+$/.test(user))
                return id;
            const pnJid = `${user}${device !== '0' ? `:${device}` : ''}@${domainType === WAJIDDomains.HOSTED ? 'hosted' : 's.whatsapp.net'}`;
            const lidForPN = await lidMapping.getLIDForPN(pnJid);
            if (lidForPN) {
                const lidAddr = jidToSignalProtocolAddress(lidForPN);
                const wireAddr = lidAddr.toString();
                logger.debug({ id, wireAddr }, 'Resolved PN Signal address to LID');
                return wireAddr;
            }
        }
        return id;
    ...
    return {
        ...
        loadSenderKey: async (senderKeyName) => {
            ...
            let { [keyId]: key } = await keys.get('sender-key', [keyId]);
            if (!key) {
                const [group, user, device] = keyId.split('::');
                if (user && device && !user.includes('-')) {
                    const wireAddr = await resolveLIDSignalAddress(`${user}.${device}`);
                    if (wireAddr !== `${user}.${device}`) {
                        const [newUser, newDevice] = wireAddr.split('.');
                        const lidKeyId = `${group}::${newUser}::${newDevice}`;
                        logger.debug({ keyId, lidKeyId }, 'SenderKey lookup: using LID-mapped key ID');
                        const { [lidKeyId]: lidKey } = await keys.get('sender-key', [lidKeyId]);
                        key = lidKey;
                    }
                }
            }
        ...
        storeSenderKey: async (senderKeyName, key) => {
            let keyId = senderKeyName.toString();
            const [group, user, device] = keyId.split('::');
            if (user && device && !user.includes('-')) {
                const wireAddr = await resolveLIDSignalAddress(`${user}.${device}`);
                if (wireAddr !== `${user}.${device}`) {
                    const [newUser, newDevice] = wireAddr.split('.');
                    const lidKeyId = `${group}::${newUser}::${newDevice}`;
                    logger.debug({ keyId, lidKeyId }, 'SenderKey storage: using LID-mapped key ID');
                    keyId = lidKeyId;
                }
            }

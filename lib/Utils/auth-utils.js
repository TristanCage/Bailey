...
export function makeCacheableSignalKeyStore(store, logger, _cache) {
    ...
    return {
        ...
        transaction: store.transaction ? store.transaction.bind(store) : undefined
    };
}
/**
...
export const addTransactionCapability = (state, logger, { maxCommitRetries, delayBetweenTriesMs }) => {
    ...
    return {
        ...
        transaction: async (work, key) => {
            ...
            const txKey = key || 'default';
            const mutex = getTxMutex(txKey);
            acquireTxMutexRef(txKey);
            try {
                return await mutex.runExclusive(async () => {
                    ...
                    try {
                        const result = state.transaction
                            ? await state.transaction(executeWork)
                            : await executeWork();

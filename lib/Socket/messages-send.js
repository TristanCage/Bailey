...
import { aggregateMessageKeysNotFromMe, assertMediaContent, bindWaitForEvent, buildTcTokenFromJid, decryptMediaRetryData, encodeNewsletterMessage, encodeSignedDeviceIdentity, encodeWAMessage, encryptMediaRetryRequest, extractDeviceJids, generateMessageIDV2, generateParticipantHashV2, generateWAMessage, getStatusCodeForMediaRetry, getUrlFromDirectPath, getWAUploadToServer, MessageRetryManager, normalizeMessageContent, parseAndInjectE2ESessions, unixTimestampSeconds } from '../Utils/index.js';
    ...
    const relayMessage = async (jid, message, { messageId: msgId, participant, additionalAttributes, additionalNodes, useUserDevicesCache, useCachedGroupMetadata, statusJidList }) => {
        ...
        await authState.keys.transaction(async () => {
            ...
            if (isGroupOrStatus && !isRetryResend) {
                ...
                logger.debug({ jid, participantsList, deviceCount: devices.length }, 'group devices resolved');
                ...
                const phashList = devices.map(d => {
                    ...
                    return `${user}${server === 's.whatsapp.net' ? '.0' : ''}:${device || 0}@${server}`;
                });
                if (isGroup) {
                    additionalAttributes = {
                        ...additionalAttributes,
                ...
                for (const device of devices) {
                    ...
                    const isOwnDevice = areJidsSameUser(deviceJid, meId) || (meLid && areJidsSameUser(deviceJid, meLid));
            ...
            if (!isNewsletter && !isRetryResend && !isStatus) {
                await buildTcTokenFromJid({
                    authState,
                    jid: destinationJid,
                    baseContent: stanza.content
                });
            }

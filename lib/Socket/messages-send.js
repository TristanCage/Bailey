...
export const makeMessagesSocket = (config) => {
    ...
    const { ev, authState, messageMutex, signalRepository, upsertMessage, query, fetchPrivacySettings, generateMessageTag, sendNode, groupMetadata, groupToggleEphemeral } = sock;
    ...
    const getUSyncDevices = async (jids, useCache, ignoreZeroDevices) => {
        ...
        if (result) {
            ...
            if (lidResults.length > 0) {
                logger.info({ count: lidResults.length }, 'Storing new LID mappings from USync device call');
                await signalRepository.lidMapping.storeLIDPNMappings(lidResults.map(a => ({ lid: a.lid, pn: a.id })));
                // Force-refresh sessions for newly mapped LIDs to align identity addressing
                try {
                    const lids = lidResults.map(a => a.lid);
                    if (lids.length) {
                        logger.debug({ lids }, 'Asserting sessions for newly mapped LIDs');
        ...
        const encryptionPromises = patchedMessages.map(async ({ recipientJid: jid, message: patchedMessage }) => {
            try {
                ...
                const node = await (async () => {
                    ...
                    // Ensure JID in 'to' node is exactly what we encrypted for
                    return {
                        tag: 'to',
                        attrs: { jid },
                        content: [
    ...
    const relayMessage = async (jid, message, { messageId: msgId, participant, additionalAttributes, additionalNodes, useUserDevicesCache, useCachedGroupMetadata, statusJidList }) => {
        const messageId = msgId || generateMessageTag();
        logger.debug({ messageId, jid, participant: participant?.jid }, 'relaying message');
        const meId = authState.creds.me.id;
        const meLid = authState.creds.me?.lid;
        ...
        // DM PN -> LID Migration (whatsmeow style)
        if (!isLid && !isGroup && !isStatus && !isNewsletter && !isRetryResend) {
            const lid = await signalRepository.lidMapping.getLIDForPN(jid);
            if (lid) {
                logger.info({ jid, lid }, 'Addressing migration: using LID for PN encryption');
                finalJid = lid;
        ...
        await authState.keys.transaction(async () => {
            ...
            if (isGroupOrStatus && !isRetryResend) {
                ...
                if (isLid && meLid) {
                    ownId = meLid;
                    logger.info({ to: jid, ownId }, 'Using LID identity for @lid conversation');
                }
                else {
                ...
                if (!isRetryResend) {
                    ...
                    if (user !== ownUser) {
                        ...
                        // Use conversation-appropriate sender identity for own device enumeration
